...
public final class SwerveModule {
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                Conversions.falconToMeters(driveMotor.getSelectedSensorPosition(), Constants.WHEEL_CIRCUMFERENCE,
                        Constants.TRACTION_GEAR_RATIO),
                getAngle());
    }

    private Rotation2d getAngle() {
        return Rotation2d.fromDegrees(
                Conversions.falconToDegrees(angleMotor.getSelectedSensorPosition(), Constants.STEERING_GEAR_RATIO));
    }
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(
                Conversions.falconToMPS(driveMotor.getSelectedSensorVelocity(), Constants.WHEEL_CIRCUMFERENCE,
                        Constants.TRACTION_GEAR_RATIO),
                getAngle());
    }
    ...
    public void setDesiredState(SwerveModuleState desiredState) {
        ...
        // return;
        // }
        // SmartDashboard.putNumber("Swerve[" + moduleName + "] desired speed",
        ...
        // SmartDashboard.putNumber("Swerve[" + moduleName + "] angle",
        // getCANCoderAngle());
        // SmartDashboard.putString("Swerve[" + moduleName + "] state",
        // desiredState.toString());
        // SmartDashboard.putString("Swerve[" + moduleName + "] state angle",
        // desiredState.angle.toString());
    }

    public void setAngle(SwerveModuleState desiredState) {
        Rotation2d angle = (Math.abs(desiredState.speedMetersPerSecond) <= (Constants.MAX_METERS_PER_SECOND * 0.01))
                ? lastAngle
                : desiredState.angle; // Prevent rotating module if speed is less then 1%. Prevents Jittering.

        angleMotor.set(ControlMode.Position,
                Conversions.degreesToFalcon(angle.getDegrees(), Constants.STEERING_GEAR_RATIO));
        lastAngle = angle;
    }

...
public class FeederStationPosition extends SequentialCommandGroup {
  /** Creates a new PlaceHigh. */
  private Elevator elevator = Elevator.getInstance();
  private Clamp clamp = Clamp.getInstance();
  public FeederStationPosition() {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new ElevatorUpMid(elevator),
      new SetClampFeederStation(clamp),
      new InstantCommand(() -> elevator.tiltElevatorUp()),
      new WaitCommand(1),
      new ElevatorFeederStation(elevator),
